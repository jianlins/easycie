...
//              support new format, configurations are listed as cpName/configName->value
    ...
    public void initCpeDescriptor(String cpeDescriptor, String annotator) {
        ...
        try {
            ...
            cpeRootFolder = new File(currentCpeDesc.getSourceUrl().getFile()).getParentFile();
            CpeCollectionReader[] collRdrs = currentCpeDesc.getAllCollectionCollectionReaders();
            for (CpeCollectionReader collReader : collRdrs) {
                File descFile = new File(cpeRootFolder + System.getProperty("file.separator") + collReader.getDescriptor().getImport().getLocation());
...
//      if external configuration is available
        System.err.println("Configure reader para: ");
        ...
        for (Map.Entry<String, String> externalConfigs : externalConfigMap.get(readerName).entrySet()) {
            ...
            System.err.println("try set "+configName+" to "+ convertValueType(valueStr, valueType));
        }
        try {
    ...
    protected void updateProcessorsDescriptorConfigurations(
        ...
        for (int i = 0; i < cpeCasProcessorsArray.length; i++) {
            ...
            File descFile = new File(cpeRootFolder + File.separator + cp.getCpeComponentDescriptor().getImport().getLocation());
...
//            updateCasProcessor(casProcessor, "typedescriptor", typeXMLAbsolutPath);
    ...
    public void run() {
        try {
            ...
            statCbL = new SimpleStatusCallbackListenerImpl(logger);
