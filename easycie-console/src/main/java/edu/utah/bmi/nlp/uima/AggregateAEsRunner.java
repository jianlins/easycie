...
import edu.utah.bmi.nlp.uima.writer.BratExporter_AE;
...
public class AggregateAEsRunner {
    ...
    public void refreshPipe(String... options) {
        ...
        logger.fine("Debug pipeline refreshed.");
...
//        JXTransformer jxTransformer;
    ...
    private void addWriter() {
        ...
        switch (option) {
            case "ehost":
                ...
                break;
            case "brat":
                runner.addAnalysisEngine(BratExporter_AE.class,
                        new Object[]{BratExporter_AE.PARAM_OUTPUTDIR, new File(output, "brat").getAbsolutePath(),
                                BratExporter_AE.PARAM_ANNOTATOR, annotator});
                break;
            case "xmi":
    ...
    public void setUIMALogger(NLPDBConsoleLogger logger) {
        uimaLogger = logger;
    }

    protected void initUIMALogger() {
        if (!option.equalsIgnoreCase("db")) {
            if (writeConfigFileName != null && writeConfigFileName.length() > 0) {
                uimaLogger = new NLPDBConsoleLogger(writeConfigFileName, annotator);
                ((NLPDBConsoleLogger) uimaLogger).setReportable(false);
            } else {
                uimaLogger = new ConsoleLogger();
            }
            uimaLogger.logStartTime();
        } else {
            this.uimaLogger = new NLPDBConsoleLogger(this.writeConfigFileName, "LOG", "RUN_ID", this.annotator);
            ((NLPDBConsoleLogger) uimaLogger).setReportable(report);
        }
    }
        ...
        for (File aeFile : aeFiles) {
            if (aeFile.isFile()) {
                ...
                if (option.equals("debug") && logTypes.length() > 0) {
                    if (uimaLogger instanceof NLPDBConsoleLogger || uimaLogger instanceof NLPDBConsoleLogger) {
    ...
    protected void initTypes(String customTypeDescriptor) {
        logger.fine("Read and initiate type system...");
        File[] aeFiles = new File(aesDir).listFiles();
        Arrays.sort(aeFiles, NAME_COMPARATOR);
        for (File aeFile : aeFiles) {
            if (aeFile.isFile()) {
