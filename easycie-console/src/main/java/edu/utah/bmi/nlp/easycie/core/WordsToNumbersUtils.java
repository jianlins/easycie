 ...
 */
public class WordsToNumbersUtils {
    ...
    public static int convertTextualNumbersToInteger(String inputText) {
        int value = 0;
        try {
            String numberStr = convertTextualNumbersInDocument(inputText);
            value = NumberUtils.createInteger(numberStr);
        } catch (Exception e) {
            System.err.println("Cannot convert text to number properly: " + inputText);
        }
        return value;
     ...
     *
     * @param inputText textual number string
    ...
    private static List<String> cleanAndTokenizeText(String sentence) {
        List<String> words = new LinkedList<String>(Arrays.asList(sentence.split("(-\\s*|\\s+)")));
