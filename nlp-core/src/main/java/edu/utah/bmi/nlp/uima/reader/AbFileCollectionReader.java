 ...
 * </ul>
 */
public class AbFileCollectionReader extends CollectionReader_ImplBase {
    /**
    ...
    protected String overWriteAnnotatorName = null;
    ...
    public void initialize() throws ResourceInitializationException {
        ...
        addFilesFromDir(directory, null);
    }
     ...
     *
     * @param dir       location of documents
     * @param extension file extension name filter
     */
    protected void addFilesFromDir(File dir, String extension) {
        File[] files = dir.listFiles();
        for (int i = 0; i < files.length; i++) {
            if (!files[i].isDirectory()) {
                if (extension == null || files[i].getName().endsWith(extension))
                    mFiles.add(files[i]);
            } else if (mRecursive) {
                addFilesFromDir(files[i], extension);
            }
        }
    }
    ...
    public void getNext(CAS aCAS) throws IOException, CollectionException {
        ...
        srcDocInfo.setUri(file.getAbsolutePath());
    ...
    protected String genURIStr(File file) {
        RecordRow recordRow = new RecordRow().addCell("DOC_NAME", file.getName())
                .addCell("ANNOTATOR", overWriteAnnotatorName);
