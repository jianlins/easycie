...
public class XMIWritter_AE extends JCasAnnotator_ImplBase {
    ...
    public void initialize(UimaContext cont) {
        String includeTypes = baseInit(cont, "data/output/xmi", "org/apache/uima");
        uimaTypes = new HashMap<>();
        if (includeTypes.length() > 0) {
            String[] types = includeTypes.split(",");
            Arrays.asList(types).forEach(e -> {
                if (e.trim().length() > 0)
                    uimaTypes.put(AnnotationOper.getTypeClass(DeterminantValueSet.checkNameSpace(e.trim())), true);
            });
        }
    }
