            ...
            if (init.get(1).substring(1).equals("RELATION_DEFINITION")) {
                ...
                for (String featureValuePairStr : init.get(3).split("\\s*,\\s*")) {
                    ...
                    if (featureValuePair.length > 2) {
                        ...
                        if (!setGetFeatureMap.containsKey(evidenceClassName))
                            setGetFeatureMap.put(evidenceClassName, new HashMap<>());
                        setGetFeatureMap.get(evidenceClassName).put(setFeatureName, getFeatureName);
                    }
                ...
                switch (featureName) {
                    ...
                    default:
                        ...
                        if (srcAnnoClassMap.containsKey(value)) {
                            ...
                            if (setGetFeatureMap.containsKey(value) && setGetFeatureMap.get(value).containsKey(featureName)) {
                                featureName = setGetFeatureMap.get(value).get(featureName);
                            }
                            if (featureName.equals("SimpleType")) {
                                evidenceConceptGetFeatures.get(value).put(featureName,
                                        AnnotationOper.getFeatureMethod(srcAnnoClassMap.get(value), "Type"));
                            } else
                                evidenceConceptGetFeatures.get(value).put(featureName,
...
//                println(ele1.getCoveredText());
                ...
                for (Object scopeObj : ele1tmp.keySet()) {
                    ...
                    if (!annoIdx.containsKey(scopeType) || annoIdx.get(scopeType).size() == 0)
                        continue;
                    LinkedHashSet<Integer> tokenIds = annoIdx.get(scopeType).get(new Interval1D(ele1.getEnd() - 1, ele1.getEnd()));
                    if (!tokenIds.iterator().hasNext())
                        continue;
                    int endScopeId = annoIdx.get(scopeType).get(new Interval1D(ele1.getEnd() - 1, ele1.getEnd())).iterator().next();
                    int beginScopeId = annoIdx.get(scopeType).get(new Interval1D(ele1.getBegin(), ele1.getBegin() + 1)).iterator().next();
                    ...
                    for (Object scopeNumObj : scopetmp.keySet()) {
                        ...
                        for (Object directionObj : numtmp.keySet()) {
                            ...
                            for (Object ele2Obj : directiontmp.keySet()) {
                                ...
                                Iterable<LinkedHashSet<Integer>> ele2s = new ArrayList<>();
                                ...
                                for (LinkedHashSet<Integer> ele2Ids : ele2s) {
                                    for (int ele2Id : ele2Ids) {
                                        ...
                                        if (directiontmp.containsKey(END)) {
                                            ...
                                            if (ele1.getBegin() < ele2.getBegin()) {
                                                int tokenScopeBegin = annoIdx.get("Token").get(new Interval1D(ele1.getEnd() - 1, ele1.getEnd())).iterator().next() + 1;
                                                int tokenScopeEnd = annoIdx.get("Token").get(new Interval1D(ele2.getBegin(), ele2.getBegin() + 1)).iterator().next();
                                                ...
                                                int tokenScopeBegin = annoIdx.get("Token").get(new Interval1D(ele2.getEnd() - 1, ele2.getEnd())).iterator().next() + 1;
                                                int tokenScopeEnd = annoIdx.get("Token").get(new Interval1D(ele1.getBegin(), ele1.getBegin() + 1)).iterator().next();
                                                logger.finest(String.format("%d: %s\t---\t%d: %s", tokenScopeBegin, annos.get("Token").get(tokenScopeBegin).getCoveredText(), tokenScopeEnd, annos.get("Token").get(tokenScopeEnd).getCoveredText()));
...
//                println(tmp.get(next));
                ...
                if (nextKey.startsWith("<") && nextKey.endsWith(">")) {
                    ...
                    Iterable<LinkedHashSet<Integer>> clues = annoIdx.get(clueType).getAll(scopeInv);
                    ...
                    for (int cludId : sortedClueIds) {
                        Integer tokenClueId = annoIdx.get("Token").get(new Interval1D(annos.get(clueType).get(cludId).getEnd() - 1, annos.get(clueType).get(cludId).getEnd())).iterator().next();
                        ...
                        if (scopeInv.max <= scopeBegin) {
                            if (logger.isLoggable(Level.FINEST)) {
                                logger.finest(annos.get("Token").get(localTokenScopeBegin).getCoveredText() + "\t" + annos.get("Token").get(annoIdx.get("Token").get(new Interval1D(scopeInv.max - 1, scopeInv.max)).iterator().next()).getCoveredText());
...
//                                TODO differentiate directions
                                if (scopeInv.max <= token.getEnd()) {
                                    if (logger.isLoggable(Level.FINEST)) {
                                        logger.finest(token.getCoveredText() + "\t" + annos.get("Token").get(annoIdx.get("Token").get(new Interval1D(scopeInv.max - 1, scopeInv.max)).iterator().next()).getCoveredText());
            ...
            for (String featureName : featureValues.keySet()) {
                switch (featureName) {
                    ...
                    default:
                        ...
                        if (srcAnnoClassMap.containsKey(srcClsName)) {
                            Class<? extends Annotation> srcCls = srcAnnoClassMap.get(srcClsName);
                            String getFeatureName = featureName;
                            if (setGetFeatureMap.containsKey(srcClsName) && setGetFeatureMap.get(srcClsName).containsKey(featureName)) {
                                getFeatureName = setGetFeatureMap.get(srcClsName).get(featureName);
                            }
                            Feature f1 = args.get(srcClsName).getType().getFeatureByBaseName(featureName);
                            if (f1 == null){
                                logger.warning("Feature: "+featureName+" doesn't exists in annotation type: "+srcCls+". " +
                                        "Please check the relation rules configuration related to "+srcCls+" where the feature "+featureName+" is assigned a value.");
                                continue;
                            }
                            Object featureValue = FSUtil.getFeature(args.get(srcClsName), f1, Object.class);
...
//                                Object featureValue = getMethod.invoke(args.get(value));
                            ...
                            FSUtil.setFeature(relationAnno, featureName,featureValue == null ? null : featureValue.toString());
                        } else {
                            FSUtil.setFeature(relationAnno, featureName, srcClsName);
                        }
                        break;
                }
            }
    ...
    protected void indexClues(JCas jCas, Set<String> types, LinkedHashMap<String, IntervalST<LinkedHashSet<Integer>>> annoIdx,
