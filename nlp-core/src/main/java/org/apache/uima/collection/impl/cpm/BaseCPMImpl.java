...
import org.apache.uima.collection.impl.cpm.CPEConfig;
...
import static edu.utah.bmi.nlp.core.DeterminantValueSet.randomString;
...
public class BaseCPMImpl implements BaseCPM, Runnable {
    ...
    public BaseCPMImpl(CpeDescription aDescriptor, ResourceManager aResourceManager,
        ...
        CpeConfiguration cpeConfig = aDescriptor.getCpeConfiguration();
        URL cpeUrl = cpeConfig.getSourceUrl();
        if (cpeUrl==null){
            File tmpCpeDesFile=File.createTempFile(RandomStringUtils.randomAlphanumeric(8),".xml");
            aDescriptor.toXML(new FileOutputStream(tmpCpeDesFile));
            aDescriptor.setSourceUrl(tmpCpeDesFile.toURI().toURL());
            this.cpeDescriptionLocation=cpeConfig.getSourceUrl().toString();
        }else {
            this.cpeDescriptionLocation = cpeUrl.toString();
        }
     ...
     *
     * @param aCollectionReader the collection reader
     */
