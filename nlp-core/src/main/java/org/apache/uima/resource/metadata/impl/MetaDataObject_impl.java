...
public abstract class MetaDataObject_impl implements MetaDataObject {
    ...
    public interface Serializer {
        ...
        boolean shouldBeSkipped(PropertyXmlInfo propInfo, Object val, org.apache.uima.resource.metadata.impl.MetaDataObject_impl mdo);

        boolean startElementProperty();

        void deleteNodeStore();

        boolean indentChildElements(XmlizationInfo info, org.apache.uima.resource.metadata.impl.MetaDataObject_impl mdo);
    ...
    MetaDataAttr[] getUnfilteredAttributes() {
        final Class<? extends org.apache.uima.resource.metadata.impl.MetaDataObject_impl> clazz = this.getClass();
    ...
    private void getAttributesFromBeans(final Class<? extends org.apache.uima.resource.metadata.impl.MetaDataObject_impl> clazz) {
     ...
     * returned (e.g. <code>java.lang.Integer</code> instead of int).
     *
     * @param aName name of an attribute
    ...
    public void setSourceUrl(URL aUrl) {
        ...
        for (MetaDataAttr attr : attrs) {
            ...
            if (val instanceof org.apache.uima.resource.metadata.impl.MetaDataObject_impl) {
                ((org.apache.uima.resource.metadata.impl.MetaDataObject_impl) val).setSourceUrl(aUrl);
            } else if (val != null && val.getClass().isArray()) {
                Object[] arrayVal = (Object[]) val;
                for (Object item : arrayVal) {
                    if (item instanceof org.apache.uima.resource.metadata.impl.MetaDataObject_impl) {
                        ((org.apache.uima.resource.metadata.impl.MetaDataObject_impl) item).setSourceUrl(aUrl);
                    }
                }
            } else if (val instanceof Map) {
                Collection values = ((Map) val).values();
                for (Object value : values) {
                    if (value instanceof org.apache.uima.resource.metadata.impl.MetaDataObject_impl) {
                        ((org.apache.uima.resource.metadata.impl.MetaDataObject_impl) value).setSourceUrl(aUrl);
                    }
                }
            }
        }
     ...
     */
    public Object clone() {
        org.apache.uima.resource.metadata.impl.MetaDataObject_impl clone = null;
        try {
            // do the default cloning
            clone = (org.apache.uima.resource.metadata.impl.MetaDataObject_impl) super.clone();
     ...
     *
     * @param aObj object with which to compare this object
     * @return true if and only if this object is equal to <code>aObj</code>
     */
    public boolean equals(Object aObj) {
        if (!(aObj instanceof org.apache.uima.resource.metadata.impl.MetaDataObject_impl)) {
            return false;
        }
        org.apache.uima.resource.metadata.impl.MetaDataObject_impl mdo = (org.apache.uima.resource.metadata.impl.MetaDataObject_impl) aObj;
...
//      String attrName = attr.name;
     ...
     * Writes out this object's XML representation.
     *
     * @param aOutputStream an OutputStream to which the XML string will be written
     */
...
//      attrs.addAttribute("", "xmlns", "xmlns", "xs:string", inf.namespace);  // NOTE:  Saxon appears to ignore this ??
     ...
     *
     * @param aXmlElementName the unqualified name of an XML element
     ...
     *
     * @param aPropInfo  information on how to represent the property in XML
     * @param aNamespace XML namespace URI for this object
     *                   representation
     * @throws SAXException -
     */
...
//      elementNode = findMatchingSubElement(aContentHandler, aPropInfo.xmlElementName);
     ...
     *
     * @param aPropName            name of the attribute
     * @param aPropClass           class of the attribute
     * @param aValue               value (guaranteed to be an array and non-null)
     * @param aArrayElementTagName name of tag to be assigned to each element of the array. May be <code>null</code>,
     *                             in which case each element will be assigned a value appropriate to its class.
     * @param aNamespace           the XML namespace URI for this object
     * @throws SAXException -
     */
     ...
     *
     * @param aPropName        name of the property to write to XML
     * @param aXmlElementName  name of the XML element for the property, <code>null</code> if none
     * @param aKeyXmlAttribute name of the XML attribute for the key
     * @param aValueTagName    XML element tag name to use for each entry in the Map
     * @param aOmitIfNull      if true, null or empty map will not be written at all, if false, null or empty map
     *                         will be written as an empty element
     * @param aNamespace       namespace for this object
     ...
     * @param aParser  a reference to the UIMA <code>XMLParser</code>. The
     ...
     *
     * @param aPropXmlInfo information about the property to read
     * @param aElement     DOM element to read from
     * @param aParser      parser to use to construct complex values
     * @param aOptions     option settings
     * @throws InvalidXMLException -
     */
     ...
     *
     * @param aPropXmlInfo information about the property to read
     * @param aPropClass   class of the property's value
     * @param aElement     DOM element representing the entire array
     * @param aParser      parser to use to construct complex values
     * @param aOptions     option settings
     * @throws InvalidXMLException -
     */
     ...
     * @param aParser             parser to use to construct complex values
     * @param aOptions            -
     * @param aKnownPropertyNames List of propertiees that we've already values for (these values will not be
     *                            overwritten)
     * @throws InvalidXMLException if no acceptable object is described by aElement
     */
     ...
     *
     * @param aPropName        name of the property to read from XML
     * @param aElement         element to read from
     * @param aKeyXmlAttribute XML attribute for the key
     * @param aValueTagName    XML element tag name for each entry in the map
     * @param aParser          parser to use to build sub-objects
     * @param aOptions         parsing option settings
     * @param aValueIsArray    true if the value of the map entires is an array. This method only supports
     *                         homogeneous arrays.
     * @throws InvalidXMLException -
     */
     ...
     *
     * @param aPrimitiveType <code>Class</code> object representing a primitive type
     ...
     *
     * @param aObj the object to write
