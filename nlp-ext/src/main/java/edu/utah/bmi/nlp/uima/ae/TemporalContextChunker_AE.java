...
public class TemporalContextChunker_AE extends TemporalContext_AE {
    ...
    public static final String PARAM_EXCLUDE_SECTIONS = "0623ExcludeSections";
...
//              Use next sentence begin to set current chunk end.
                    ...
                    if (!sentenceTree.contains(new Interval1D(date.getBegin(), date.getEnd())))
                        continue;
...
//                    System.out.println(chunkBegin+"~"+currentSentence.getBegin());
                        ...
                        if (chunkBegin < currentSentence.getBegin() - 1) {
                            ...
                            if (i == 0) {
                                java.util.Date firstDate = new org.pojava.datetime.DateTime(date.getNormDate(), DateTimeConfig.getDateTimeConfig(recordDate.toDate())).toDate();
                                if (firstDate.after(recordDate.toDate())) {
                                    logger.finest(date.getNormDate() + ": is the 1st date mention in this chunk, but it's later than the note record date. So the subchunk before the 1st date will be considered to be affected by the note date.");
                                } else {
            ...
            logger.warning(metaRecordRow.toString());
            e.printStackTrace();
        }
    ...
    private void indexFilterConcepts(JCas jCas, LinkedHashMap<String, Class<? extends
            Annotation>> targetConceptClasses,
                                     IntervalST<Integer> filteredConceptTree, ArrayList<Annotation> filteredConcepts,
        ...
        for (Class<? extends Annotation> cls : targetConceptClasses.values()) {
            ...
            while (annoIter.hasNext()) {
                ...
                if (concept.getTemporality() != null && (!concept.getTemporality().equals("present"))
                        && (!concept.getTemporality().equals("historical")))
                    continue;
